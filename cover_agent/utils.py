...
def extract_yaml_content(response_text: str) -> str:
    """
    Extract YAML content from a response text that may contain additional text.

    Parameters:
    response_text (str): The response text containing YAML data.

    Returns:
    str: The extracted YAML content or an empty string if no YAML block is found.
    """
    # Regular expression to match YAML block inside triple backticks or at the start
    yaml_pattern = re.search(r"```yaml(.*?)```", response_text, re.DOTALL)
    
    if yaml_pattern:
        return yaml_pattern.group(1).strip()
